...
export function SoftSettingsPage() {
  ...
  const isClient = useAuthStore((s) => s.role) === 'client'
        ...
        <div className="settings-row" style={{ marginBottom: 12 }}>
          <div className="settings-row-label">
            ...
            <Typography.Text type="secondary">
              {isClient
                ? '多久从服务端拉取一次设备与状态数据。客户端不直连打印机；真正连接与采集由服务端完成。'
                : '轮询类设备多久更新一次状态（Klipper 备用拉取、闪铸 / Snapmaker / 纵维等）。拓竹 MQTT 推送不受影响。修改后会重连设备。'}
            </Typography.Text>
          </div>
          <Select
            style={{ width: 120 }}
            ...
            onChange={(v) => {
              void (async () => {
                ...
                if (isClient) {
                  message.success('已保存，将按新间隔从服务端刷新数据')
                } else {
